...
class Customer_info:
    def __init__(self, customer_name="", customer_nickname="", customer_phone="",
        ...
        if not all(x.isalpha() or x.isspace() for x in customer_nickname):
            raise ValueError("Your nickname should be string.")
        if not all(x.isnumeric() or x.isspace() for x in customer_phone):
            raise ValueError("Your phone number should be numeric.")
        if not all(x.isnumeric() or x.isspace() for x in customer_phone) and \
    ...
    def cancel(self, name):
        try:
            ...
            if name in data:
                ...
                print(f"No data for account number: {self.customer_name}")
